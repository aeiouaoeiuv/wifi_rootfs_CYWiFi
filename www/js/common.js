	...
	var MODEL = new Array("其它","华为","小米","魅族","磊科","联想","长虹","HTC","苹果","三星","诺基亚","索尼","酷派","OPPO","LG","中兴","步步高VIVO","摩托罗拉","金立","天语","TCL","飞利浦","戴尔","惠普","华硕","东芝","宏碁","海信","腾达","TPLINK","一加","锤子","瑞昱","清华同方","触云","创维","康佳","乐视","海尔","夏普","VMware","Intel","技嘉","海华","其它","其它","其它","其它","仁宝","其它","糖葫芦","其它","其它","谷歌","天珑","泛泰","其它","其它","微软","极路由","奇虎360","水星","华勤","安利","富士施乐","D-Link","网件","阿里巴巴","微星","TCT");
	...
	*/
	function getErrorCode(type){
		switch(type){
			...
			case 20 : return "运营商拨号无响应！"; break;
			default : return "请求超时，错误码："+type;
		}
	}
				 ...
				 success: function(data){
					...
					if(jsonObject.error == 0){
						if(jsonObject.enable == 0){
							...
							$("#totalSpeed").html((jsonObject.down / 1024).toFixed(0) + "M");
						}
					}
				 }
			 });
				 ...
				 success: function(data){
					...
					if(jsonObject.error == 0){
						if(jsonObject.connected == true){
							...
							document.getElementById("netStatus").innerHTML = "未连接&nbsp;&nbsp;<input onclick=\"javascript:document.location='admin.html?c=1&t='+new Date().getTime();\" type=\"button\" value=\"连接\">";
						}
						...
						if(terminals != null && terminals.length > 0){
							for(var i=0; i<terminals.length; i++){
								...
								if(flag.charCodeAt(1) == 84){	//T
									...
									var used_time = "时长："+(terminals[i].ontime / 3600).toFixed(2) + "小时";
